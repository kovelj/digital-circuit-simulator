...
RucniGen::RucniGen(std::fstream& inputFile, std::vector<float>& vreme, float trajanje, int id, int tip) : Element(id, tip, 0,0,0)
{
	float promena = 0;
	...
	int velicina = vreme.size();
		while(inputFile.peek()!='\n') {
			...
			if (korak < 0)  {
				throw std::logic_error("Simulation step has to be positive.");
			}

			promena += korak;

			if (korak > trajanje || korak <= 0) {
				throw std::logic_error("Simulation step can't be greater than simulation duration.");
;
			}
...
TaktGen::TaktGen(std::fstream& inputFile, std::vector<float>& vreme,float trajanje , int id, int tip) : Element(id,tip,0,0,0)
{
...
IliKolo::IliKolo(std::fstream& inputFile, int id, int tip) : Element(id, tip,2,0,0)
{
	inputFile >> broj_pinova_;
	if (broj_pinova_ <= 0)  {
		throw std::invalid_argument("OR circuit recquires positive number of inputs.");
	}
...
{//ovde je greska jer je ulsov los cak ni sam ne znam sta radi
	...
	if (pin<broj_pinova_) {
		...
		throw std::logic_error("pin does not exist, can't connect. Check input file.");
	}
}

IKolo::IKolo(std::fstream& inputFile, int id,int tip) :Element(id, tip,2,0,0)
{
	inputFile >> broj_pinova_;
	for (int i = 0; i < broj_pinova_; i++) {
		sinovi_.push_back(nullptr);
...
NeKolo::NeKolo(std::fstream& inputFile, int id,int tip) :Element(id, tip,1,0,0)
{
	for (int i = 0; i < broj_pinova_; i++) {
		sinovi_.push_back(nullptr);
	}
...
Element::Element(int id, int tip,int podrazumevano,int neispitan,int broj_prolazaka=0)
	: id_(id), broj_pinova_(podrazumevano), vrednost_(0), frekvencija_(0), broj_prolazaka_(broj_prolazaka)
{
	switch (tip) {
...
bool Element::vecObidjen()
{
	...
	return broj_prolazaka_ == sinovi_.size();
}
